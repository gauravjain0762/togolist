...
import {FullWindowOverlay} from 'react-native-screens'; // Import FullWindowOverlay
...
const SharedListDetails = () => {
    ...
    return (
      <View
        style={[
          ...
          {marginBottom: isVisible ? 10 : 0, bottom: Insets.bottom},
        ]}>
        <TouchableOpacity
          onPress={() => {
            setSelect('Map View');
  ...
  const renderItem = ({item}) => (
    <PlacesCard
      ...
      locationIcon
      onCardPress={() => {
        cardRefs.current[item.id]?.measure(
  ...
  return (
    <View style={AppStyles.flex1}>
            ...
            <Reanimated.ScrollView
              ...
              <SwipeListView
                ...
                renderItem={(data, rowMap) => {
                  return (
                    <View style={styles.rowFront}>{renderItem(data)}</View>
                  );
                }}
                disableRightSwipe
      ...
      {/* Render FullWindowOverlay outside SafeAreaView */}
      {activeCard && (
        <FullWindowOverlay>
          <TouchableOpacity
          ...
          <MotiView
            from={{
              top: activeCard.layout.y,
              left: activeCard.layout.x,
              width: activeCard.layout.width,
              height: activeCard.layout.height,
              borderRadius: 16,
            }}
            animate={{
              top: Insets.top,
            ...
            style={[styles.overlayCard, {zIndex: 20}]}>
            <CustomHeader
              backImg={IMAGES.back1}
              showBack={true}
              backIconStyle={styles.back}
                ...
                renderItem={({item}) => {
                  return (
                    <Reanimated.ScrollView
                      onScroll={scrollHandler}
                      showsVerticalScrollIndicator={false}
                      style={[styles.scroll1, {marginBottom: Insets.bottom}]}>
                      <ImageBackground
                        source={IMAGES.bbq}
                      ...
                      <View style={[{marginVertical: hp(8), gap: hp(8)}]}>
                        <LinearView containerStyle={styles.linearview}>
                          <Text style={styles.review}>
                            Fashion Valley is an upscale, open-air shopping mall
                            in Mission Valley in San Diego, California. The
                            shopping center hosts 1,720,533 sq ft of leasable
                            floor area, making it the largest mall in San Diego
                            and one of the...
                          </Text>
                              ...
                              return (
                                <>
                                  ...
                                  <Text style={styles.contentText}>
                                    You know it’s a good California burrito when
                                    you don’t need to as for guac because it’s
                                    already included. Best Cali burrito in SD
                                    imo. Their machaca burrito is killer too.
                                  </Text>
                                </>
                              );
                            })}
                          ...
                          <FlatList
                            ...
                            renderItem={({item}) => (
                              <TravelCard
                                {...item}
                                onPress={() =>
                                  navigateTo(SCREENS.GalleryScreen)
                                }
...
const styles = StyleSheet.create({
  ...
  expandedText: {
    ...
    textAlign: 'center',
    marginBottom: hp(10), // Added margin
  },
  expandedImage: {
    width: '100%',
    height: hp(200), // Adjust height as needed
    resizeMode: 'cover',
    borderRadius: 8,
    marginBottom: hp(10),
  },
